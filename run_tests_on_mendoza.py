...
def handle_one_model(model_path):
    ...
    simplified_results = simplify_test_results(test_results)
...
while True:
    try:
        ...
        print(f'Finished testing {model_paths[i]} ({i+1}/len(model_paths))')
        ...
        msg = f'Error when testing {model_paths[i]}: {error}'
